	...
	public List<BoardLocation> evaluate(Board board, Piece piece) {
		...
		if (king.getRank() == current.getRank()) {
			if (king.getFile() > current.getFile()) {
				for (int i = current.getFile(); i <= king
						.getFile(); i++) {
					boolean isKing = Move.isKing(board, king.getRank(), i, piece
				...
				for (int i = king.getFile(); i >= current
						.getFile(); i--) {
					boolean isKing = Move.isKing(board, i, king.getRank(), piece
							.getPlayer().getOpponent());
		...
		if (king.getFile() == current.getFile()) {
			if (king.getRank() > current.getRank()) {
				for (int i = current.getRank(); i <= king
						.getRank(); i++) {
					boolean isKing = Move.isKing(board, i, king.getFile(), piece
							.getPlayer().getOpponent());
				...
				for (int i = king.getRank(); i >= current
						.getRank(); i--) {
					boolean isKing = Move.isKing(board, i, king.getFile(), piece
							.getPlayer().getOpponent());
		...
		for (int i = current.getFile(), j = current
				.getRank(); i >= 0; i--, j++) {
		...
		for (int i = current.getRank(), j = current
